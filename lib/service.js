...
var methods = {
  ...
  saveSimple(dbName, data, callback, completeFn){
    return db.save(dbName, schemaMap[dbName], data, callback, completeFn);
  },
  save(type, project, data, callback, completeFn){
    return db.save(`${type}${project ? '-' + project : ''}`, schemaMap[type], data, callback, completeFn);
  },
  ...
  travelAll(type, project, callback, completeFn, filterFn){
    db.travelAll(`${type}${project ? '-' + project : ''}`, schemaMap[type], callback, completeFn, filterFn);
  },
